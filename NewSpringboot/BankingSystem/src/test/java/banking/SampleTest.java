...
public class SampleTest {
    ...
    public void testAccountCreation() {
        Assert.assertNotNull("Amelia Pond account should be created", ameliaPond);
        Assert.assertNotNull("Rose Tyler account should be created", roseTyler);
        Assert.assertNotNull("Acme Corp account should be created", acmeCorp);
        Assert.assertNotNull("HackerRank account should be created", hackerRank);
    }

    @Test
    public void testBalance() {
        Assert.assertEquals("Amelia Pond account balance should be 0.0", 0.0, bank.getBalance(ameliaPond), 0.0);
        Assert.assertEquals("Rose Tyler account balance should be 456.78", 456.78, bank.getBalance(roseTyler), 0.0);
        Assert.assertEquals("Acme Corp account balance should be 0.0", 0.0, bank.getBalance(acmeCorp), 0.0);
        Assert.assertEquals("HackerRank account balance should be 9876543.21", 9876543.21, bank.getBalance(hackerRank), 0.0);
    }
    ...
    public void testCreditAndDebit() {
        double amount = 100.0;
        bank.credit(ameliaPond, amount);
        Assert.assertEquals("Amelia Pond account balance should be credited by 100.0", amount, bank.getBalance(ameliaPond), 0.0);

        amount = 50.0;
        bank.debit(roseTyler, amount);
        Assert.assertEquals("Rose Tyler account balance should be debited by 50.0", 456.78 - amount, bank.getBalance(roseTyler), 0.0);
    }

    @Test
    public void testAuthenticateUser() {
        Assert.assertTrue("Authentication should pass for Amelia Pond", bank.authenticateUser(ameliaPond, 1111));
        Assert.assertTrue("Authentication should pass for Acme Corp", bank.authenticateUser(acmeCorp, 3333));
        Assert.assertFalse("Authentication should fail for Rose Tyler", bank.authenticateUser(roseTyler, 1234));
    }
}
